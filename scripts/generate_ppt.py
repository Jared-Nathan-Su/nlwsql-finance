...
for i,(t,d) in enumerate([("防删库跑路","系统只允许查询操作，删除、修改、建表等危险命令全部拦截"),
    ("防胡说八道","AI可能编造不存在的表名和字段，白名单机制确保只查真实数据"),("自动修正","生成出错不要紧，系统会把错误反馈给AI让它重新生成，最多试3次"),
    ("实战效果","超过80%的错误能在3次重试内自动修复，用户无感知")]):
...
mtxt(sl,1.1,3.85,11,2.1,["假设一个10人管理团队，平均每人每月需要查5次数","",
    "原来的方式: 每次查数平均花2小时等人沟通、写SQL、出报表，人力成本约20,000元/月，一年24万",
    "用AI的方式: 自己打字提问3秒出结果，大模型API每月约500元，加上运维约2,500元/月，一年仅3万","",
    "一年下来省了21万元，投1块省7块，还不算决策更快带来的隐形收益"],11,TX,Pt(4))
